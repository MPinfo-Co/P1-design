...
> **用途**：記錄 MP-BOX 所有產品功能的代碼、實作範疇、對應程式碼與完成狀態。
> 每個功能對應一個 Epic，是 SA / SD / PG Issue 的對照基準。

## 功能列表

> 編號格式 `f-{domain}-{NN}`（規則詳見下方）；完成狀態：`未開始`（無明確需求）/ `規劃中`（需求明確，待實作）/ `進行中`（實作中，含 mock 階段）/ `已完成`（前後端串接完整）
...
- **檔名同步**：code 檔案若屬於某功能編號，遵循「編號在 code 中的引用方式」小節約定；既有檔案改名見 [MPinfo-Co/P1-code#93](https://github.com/MPinfo-Co/P1-code/issues/93)
