...
| # | 類型 | 工作內容 | 參照規格 |
|---|------|--------|--------|
| 1 | API | 確認 fn_role_query_api：GET /api/roles 查詢角色清單（含角色名稱、成員、可用 AI 夥伴名稱） | Spec/fn_role/Api/fn_role_query_api.md |
| 2 | API | 確認 fn_role_add_api：POST /api/roles 新增角色（含角色名稱、成員、可用 AI 夥伴） | Spec/fn_role/Api/fn_role_add_api.md |
| 3 | API | 確認 fn_role_update_api：PATCH /api/roles/{name} 修改角色（含角色名稱、成員、可用 AI 夥伴） | Spec/fn_role/Api/fn_role_update_api.md |
...
| 6 | 畫面 | 確認 fn_role_01_list：角色清單畫面（關鍵字篩選、角色名稱、可用 AI 夥伴欄位、修改／刪除按鈕） | Spec/fn_role/fn_role_01_list.md |
| 7 | 畫面 | 確認 fn_role_02_form：角色表單畫面（角色名稱、成員、可用 AI 夥伴；停用中夥伴顯示標示且不可勾選） | Spec/fn_role/fn_role_02_form.md |
...
| ID | 類型 | 前置條件 | 操作 | 預期結果 |
|----|------|--------|------|--------|
| T1 | API | 已登入且具 `can_manage_roles` 權限，DB 有多筆角色 | GET /api/roles | 200，data 包含每筆角色名稱及可用 AI 夥伴陣列 |
...
| T10 | API | 已登入且具 `can_manage_roles` 權限，角色「待刪角色」存在 | DELETE /api/roles/待刪角色 | 200 刪除成功，tb_roles 及關聯資料表（tb_user_roles、tb_role_ai_partners）對應紀錄移除 |
...
| T15 | 畫面 | 已登入且具 `can_manage_roles` 權限 | 點擊 [新增角色] → 填入角色名稱 → 勾選成員與可用 AI 夥伴 → 點擊 [儲存] | 表單關閉，清單刷新，新角色出現於清單末尾 |
| T16 | 畫面 | 已登入且具 `can_manage_roles` 權限，清單有角色資料 | 點擊某角色的 [修改] → 修改角色名稱 → 點擊 [儲存] | 表單帶入原有資料（含成員與可用 AI 夥伴勾選狀態）；儲存後表單關閉，清單刷新，角色名稱已更新 |
| T17 | 畫面 | 已登入且具 `can_manage_roles` 權限，清單有角色資料 | 點擊某角色的 [刪除] → 確認提示彈出 → 點擊 [確認刪除] | 清單中該角色移除；點擊 [取消] 則關閉提示，清單不變 |
| T18 | 畫面 | 已登入且具 `can_manage_roles` 權限，系統中存在至少一個停用中的 AI 夥伴 | 開啟角色表單（新增或修改模式） | **可用 AI 夥伴**清單中，停用中夥伴顯示「停用中」標示，且其 checkbox 不可勾選；啟用中夥伴可正常勾選 |
