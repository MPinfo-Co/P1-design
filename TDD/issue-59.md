...
| # | 類型 | 說明 |
|---|------|------|
| 1 | Schema | 新增 `token_blacklist` 表（users 表已存在，無需異動） |
...
| T5 | API | 持有有效 JWT | POST /auth/logout | 200，token 加入黑名單 |
| T6 | API | 無 Authorization header | POST /auth/logout | 401，`請重新登入` |
| T7 | 其他 | 持有有效 JWT | 呼叫需驗證的 API | 正常通過，回傳資料 |
| T8 | 其他 | 持有已登出（黑名單）或過期的 JWT | 呼叫需驗證的 API | 401，`請重新登入` |
