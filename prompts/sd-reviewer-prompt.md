...
#### 檢核 6：畫面規格的 checkbox/select 選項來源

從工作項目中取出所有類型為 `畫面` 的項目，讀取對應的畫面規格檔案，確認其中 `checkbox group` / `select` 類型欄位是否已標明選項來源（格式：`> **{欄位}** 選項來源：\`Api/...\``）。
若有欄位缺少選項來源說明 → 記錄 ISSUE，指出哪個畫面規格的哪個欄位缺少。

#### 檢核 7：跨功能 `_test_api.md` 同步

若 TDD 工作項目中有修改其他功能 API 行為（例如修改了 `fn_user` 的 API 行為，但本次 SD 是 `fn_role` issue），確認對應功能的 `_test_api.md` 也有列入更新範圍（在 TDD Test 類型工作項目中引用）。若缺少 → 記錄 ISSUE，指出哪個功能的 `_test_api.md` 未同步。
