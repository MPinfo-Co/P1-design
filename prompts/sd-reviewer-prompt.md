...
#### 檢核 5：若有 API 新增或修改，對應 `_test_api.md` 存在

掃描工作項目中類型為 `API` 且工作內容非「刪除」的項目，依工作內容或參照規格欄位推斷功能模組名稱（`fn_xxx`），確認 `Spec/{fn_xxx}/Api/_{fn_xxx}_test_api.md` 存在。若不存在 → 記錄 ISSUE，指出缺少的檔案路徑。
