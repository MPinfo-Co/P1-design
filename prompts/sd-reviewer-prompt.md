...
3. 讀取 `P1-project/docs/spec-guide.md`（用於 Spec 格式合規審查）。
...
#### 檢核 6：Spec 產出符合 spec-guide.md 格式規範

從工作項目中取出所有類型為 `API` 或 `畫面` 的項目，讀取對應的 Spec 檔案，依 `spec-guide.md` 逐項確認格式是否合規（含 section 結構、表格欄標、畫面規格選項來源等）。若有不符 → 記錄 ISSUE，指出哪個檔案的哪個項目不符規範。
